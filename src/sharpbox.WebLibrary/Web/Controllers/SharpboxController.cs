...
namespace sharpbox.WebLibrary.Web.Controllers
{
        ...
        where T : class, new()
    {
        ...
        {
            var result = (IList<T>)this.WebContext.Dispatch.Fetch<T>(AppContext.Get, null);
            ...
            if (!string.IsNullOrEmpty(this.Request.Url?.Query))
            {
                ...
                var queryResults = options.ApplyTo(result.AsQueryable());

                result = ((IQueryable<T>)queryResults).ToList();
            }
        ...
        public virtual JsonResult GetAsLookUpDictionary()
        {
            ...
            var items = (IList<T>)this.WebContext.Dispatch.Fetch<T>(AppContext.Get, new object[] { options });
        ...
        /// <summary>
        /// Every time a request comes in we want pull the latest application info into our DefaultContext
        /// </summary>
        /// <param name="defaultContext">The DefaultContext which handles each request.</param>
        private void WarmBootAppContext(AppContext defaultContext)
        {
            this.WireDomain();
