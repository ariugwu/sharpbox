...
(function (sharpbox) {
    ...
    (function (Web) {
        ...
        (function (Templating) {
            ...
            Templating.fieldsToForm = function (viewModel) {
            ...
            Templating.searchDash = function (viewModel) {
                ...
                var searchFields = Templating.fieldsToSearch(viewModel);
                $.each(searchFields, function (key, group) {
                    html += group;
                });
                return html + ("<hr />\n                        <p class=\"text-muted bold\"><small>*This form created dynamically based on assumptions about your model.</small></p>\n                        <input type=\"hidden\" id=\"WebRequest_UiAction\" name=\"WebRequest.UiAction.Name\" value=\"" + viewModel.form.uiAction + "\" />\n                        " + viewModel.form.footer.toHtml(viewModel.form.header.name) + "\n                       </div>");
            };
            Templating.fieldsToSearch = function (viewModel) {
                var fields = viewModel.form.fieldDictionaryToArray();
                var search = [];
                var htmlStrategy = viewModel.form.htmlStrategy;
                search = $.map(fields, function (key, index) {
                    console.log(index);
                    var label = htmlStrategy.labelHtml(key, "");
                    var input = htmlStrategy.createSearchField(key);
