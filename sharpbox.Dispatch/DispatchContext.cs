...
namespace sharpbox.Dispatch
{
    ...
    public class DispatchContext : IDispatchContext
    {
        ...
        public void Register<T>(RoutineName routineName, CommandName commandName, EventName eventName, Func<T, T> action, Func<T, T> failOver = null, Func<T, T> rollBack = null)
        {
            try
            {
        ...
        public T Run<T>(RoutineName routineName, string message, object[] args)
        {
            ...
            for (var i = 0; i < routine.Count - 1; i++)
            {
                try
                {
                    result = this.Run<T>(routineName, routine[i], message + "(Routine: " + routineName + ")(" + (i + 1) + "/" + routine.Count + ")", args);
        ...
        public IQueryable<T> Fetch<T>(QueryName queryName, object[] args)
        {
        ...
        private T Run<T>(RoutineName routineName, IRoutineItem r, string message, object[] args)
        {
